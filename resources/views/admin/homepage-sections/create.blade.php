...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-white">New Homepage Section</h1>
        <p class="text-sm text-zinc-500">Tambah section dan urutannya di homepage.</p>
    </div>
</div>

<form method="POST" action="{{ route('admin.homepage-sections.store') }}" class="space-y-5">
    @csrf
    @if (request('drawer'))
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            ...
            <select name="key" class="mt-2 w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-sm text-white" required>
                ...
                @foreach ($availableSections as $key => $label)
                    <option value="{{ $key }}" @selected(old('key') === $key)>{{ $label }} ({{ $key }})</option>
                @endforeach
            </select>
        </div>
        <div>
        ...
        <div class="md:col-span-2">
            ...
            <input name="name" value="{{ old('name') }}" class="mt-2 w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-sm text-white" placeholder="Contoh: Hero + Stats" required>
        </div>
        <div class="md:col-span-2">
