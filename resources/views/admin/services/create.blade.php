...
<form method="POST" action="{{ route('admin.services.store') }}" enctype="multipart/form-data" class="space-y-5">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            ...
            <input name="label" value="{{ old('label') }}" class="mt-2 w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-sm text-white" placeholder="Contoh: SaaS Build" required>
        </div>
        <div>
        ...
        <div class="md:col-span-2">
            ...
            <input name="title" value="{{ old('title') }}" class="mt-2 w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-sm text-white" placeholder="Contoh: SaaS MVP" required>
        </div>
        <div class="md:col-span-2">
            <label class="text-xs uppercase tracking-widest text-zinc-500">Price Range</label>
            <input name="price" value="{{ old('price') }}" class="mt-2 w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-sm text-white" placeholder="Contoh: Rp 2 - 3jt">
        </div>
        <div class="md:col-span-2">
        ...
        <div class="md:col-span-2">
            ...
            <textarea name="description" rows="4" class="mt-2 w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-sm text-white">{{ old('description') }}</textarea>
        </div>
        <div class="md:col-span-2">
