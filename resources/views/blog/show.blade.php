    ...
    <section class="flex flex-col">
        ...
        <div class="max-w-5xl mb-12 reveal-up" data-reveal data-reveal-delay="40">
            <div class="relative overflow-hidden rounded-3xl border border-white/10 bg-zinc-900/40">
                ...
                <img src="{{ $post->image }}" alt="{{ $post->title }}" class="w-full h-64 md:h-80 object-cover opacity-90">
            </div>
        </div>
            ...
            <div class="lg:col-span-8 space-y-10 reveal-up" data-reveal data-reveal-delay="60">
                ...
                @foreach ($post->sections ?? [] as $section)
                    ...
                    <div class="rounded-2xl border border-white/10 bg-zinc-900/40 p-5">
                        <div class="text-[10px] uppercase tracking-widest text-emerald-400 mb-3">Baca Juga</div>
                        <div class="space-y-3">
                ...
                <div class="rounded-2xl border border-white/10 bg-zinc-900/40 p-6 text-sm text-zinc-400">
                    <div class="text-[10px] uppercase tracking-widest text-emerald-400 mb-2">Takeaway</div>
                    Fokus pada struktur dan konsistensi akan mengurangi biaya komunikasi dan membuat tim lebih cepat mengambil keputusan.
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @foreach ($posts->take(2) as $related)
                            <article class="group relative bg-zinc-900/20 border border-white/5 rounded-2xl p-5 hover:border-white/15 transition-all">
                                ...
                                <a href="{{ route('blog.show', $related->slug) }}" class="inline-flex items-center gap-2 text-xs uppercase tracking-widest text-emerald-400 hover:text-emerald-300">
                    ...
                    <div class="space-y-4">
                        @foreach ($posts->take(8) as $popular)
                            <a href="{{ route('blog.show', $popular->slug) }}" class="group flex items-start gap-4">
                                <div class="w-10 h-10 rounded-xl border border-white/10 bg-white/5 flex items-center justify-center text-xs font-semibold text-emerald-400">
                                    {{ str_pad($loop->iteration, 2, '0', STR_PAD_LEFT) }}
                                </div>
                                <div class="flex-1">
                                    <div class="text-sm font-semibold text-white group-hover:text-emerald-300 transition-colors">{{ $popular->title }}</div>
